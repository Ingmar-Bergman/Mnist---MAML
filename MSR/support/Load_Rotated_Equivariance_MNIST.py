...
#(Il reste à fusionner au début training set et test set, FAIT NORMALEMENT ), faire les transformations puis PRENDRE LE MËME NOMBRE D'ELEMENT POUR CHAQUE CLASSE 
#SINON CA BuG 
...
class MNISTNShot:
    ...
    def __init__(self, dataset, batchsz, n_way, k_shot, k_query, imgsz,num_elements_per_class, device=None):
        """
        ...
        # Create a dictionary under the form {label:img1, img2..., 20 imgs, label2: img1, img2,...}
        temp = dict()
    ...
    def load_data_cache(self, data_pack):
        ...
        for sample in range(10):  # num of episodes
            ...
            batch_label_mappings = []  # confusion/ List to keep track of label mappings for the batch
            ...
            for i in range(self.batchsz):  # one batch means one set #numero task
                ...
                label_mapping = {}  # confusion/ Dictionary to keep track of label mappings for the task
                
                for j, cur_class in enumerate(selected_cls):

                    selected_img = np.random.choice(self.num_elements_per_class, self.k_shot + self.k_query, False) # Sélectionne de manière aléatoire "self.k_shot + self.k_query" images parmi les 20 de la classe courante
                    ...
                    # confusion /Update the label mapping dictionary
                    label_mapping[j] = cur_class
                ...
                #confusion 
                # Append label_mapping for the current task
                batch_label_mappings.append(label_mapping)
            ...
            data_cache.append([x_spts, y_spts, x_qrys, y_qrys, batch_label_mappings])
